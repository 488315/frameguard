...
- `src/app/store.ts` is the observable production interface shared by React and WebMCP. It owns only UI activity and WebMCP availability state.
- `src/webmcp` declares exact browser tool schemas, validates all runtime inputs, and delegates every operation to the store. Review-only registrations use one `AbortController` per active proposal.
...
- Tool results are returned only after the shared store has emitted the visible state update.
